...
---------------------------------------------------------------*/
    ...
    <h3 id="name" class="card-name cap">${data.results[i].name.first} ${
          data.results[i].name.last
        }</h3>
    <p class="card-text">${data.results[i].email}</p>
    <p class="card-text cap">${data.results[i].location.city}, ${
...
---------------------------------------------------------------*/
      $(".card").click(function(evt) {
        ...
        <img class="modal-img" src="${
          data.results[evt.currentTarget.id].picture.large
        }" alt="profile picture">
        <h3 id="name" class="modal-name cap">${
          data.results[evt.currentTarget.id].name.first
        } ${data.results[evt.currentTarget.id].name.last}</h3>
        <p class="modal-text">${data.results[evt.currentTarget.id].email}</p>
        <p class="modal-text cap">${
          data.results[evt.currentTarget.id].location.city
        }</p>
        <hr>
        <p class="modal-text">${data.results[evt.currentTarget.id].phone}</p>
        <p class="modal-text">${
          data.results[evt.currentTarget.id].location.street
        }, ${data.results[evt.currentTarget.id].location.city}, ${
            data.results[evt.currentTarget.id].location.state
          } ${data.results[evt.currentTarget.id].location.postcode}</p>
        <p class="modal-text">Birthday:${
          data.results[evt.currentTarget.id].dob.date
        }</p>
        </div>
        </div> `;
          $("body").append(modal);
